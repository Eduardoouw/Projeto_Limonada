...
const MUSICAS = [ /* ... (todas as músicas que você já tinha) ... */ ];

// === MAPEAMENTOS === (noteLabelMap, keyMap, etc - mantenha tudo igual)

// === GRAVAÇÃO E ÁUDIO === (todo o seu código original até o final)

// === CHAT DE AJUDA - NOVO E PERFEITO ===
document.addEventListener('DOMContentLoaded', () => {
    const conversationNodes = {
        start: {
            message: 'Olá! Como posso te ajudar com o Piano Virtual?',
            choices: [
                { text: 'Como tocar com o teclado?', nextNodeId: 'teclado' },
                { text: 'Como gravar uma música?', nextNodeId: 'gravar' },
                { text: 'Como mudar a cor das teclas?', nextNodeId: 'cor' },
                { text: 'O som não está funcionando', nextNodeId: 'som' },
                { text: 'Quero falar com um humano', nextNodeId: 'humano' }
            ]
        },
        teclado: {
            message: 'Use as teclas do seu teclado físico!<br><br>• a s d f g h j → C4 a B4<br>• z x c v b n m → C3 a B3<br>• q w e r t y u → C5 a B5<br><br>Teclas pretas: segure SHIFT + número/letra',
            choices: [{ text: 'Entendi!', nextNodeId: 'start' }]
        },
        gravar: {
            message: 'Clique em "Record Gravar" → toque sua música → clique novamente para parar.<br><br>Depois use "Play Tocar" ou "Download Baixar"',
            choices: [{ text: 'Perfeito!', nextNodeId: 'start' }]
        },
        cor: {
            message: 'Abra o menu (≡) → Configurações → use o seletor de cor ao lado de "Cor das teclas brancas"',
            choices: [{ text: 'Valeu!', nextNodeId: 'start' }]
        },
        som: {
            message: 'Clique em qualquer tecla do piano primeiro!<br><br>O navegador só libera o som após uma interação do usuário (é regra de segurança).<br><br>Depois disso, tudo funciona normalmente!',
            choices: [{ text: 'Agora deu certo!', nextNodeId: 'start' }]
        },
        humano: {
            message: 'Hehe infelizmente sou só um robô muito bem feito<br><br>Mas pode mandar seu feedback para:<br><strong>contato@pianovirtual.com</strong>',
            choices: [{ text: 'Haha tudo bem!', nextNodeId: 'start' }]
        }
    };
    ...
    function showChoices(node) {
        ...
        node.choices.forEach(c => {
            ...
            btn.onclick = () => {
                addMessage(c.text, 'user');
                currentNodeId = c.nextNodeId;
                addMessage(conversationNodes[currentNodeId].message, 'bot');
                showChoices(conversationNodes[currentNodeId]);
            };
    ...
    const observer = new MutationObserver(() => {
        if (ajudaModal.classList.contains('open')) {
            setTimeout(() => {
                const display = document.getElementById('conversation-display');
                display.innerHTML = '<div class="message bot-message">Olá! Bem-vindo(a) ao Piano Virtual!<br>Como posso te ajudar hoje?</div>';
...
// === RESTANTE DO SEU SCRIPT ORIGINAL (renderPiano, controles, etc) ===
